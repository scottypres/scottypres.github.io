...
export function calculateDiesel(inputs, isReal = false) {
  try {
    const gas = getGasConstants(inputs.substance || 'air');
    const r = inputs.r ?? inputs.compression_ratio ?? 18;
    const T_1 = toKelvinAuto(inputs.T_1 ?? inputs.T1 ?? 300);
    const P_1 = inputs.P_1 ?? inputs.P1 ?? 100;
    const Q_in = inputs.Q_in ?? 1800;
    const r_c_input = inputs.r_c ?? inputs.cutoff_ratio ?? null;
    ...
    const T_3 = T_2 + Q_in / gas.cp;
    const r_c = r_c_input ?? (T_3 / T_2);
    const P_3 = P_2;
    const T_4 = T_3 * Math.pow(r_c / r, gas.k - 1);
    const P_4 = P_3 * Math.pow(r_c / r, gas.k);
    ...
    const Q_H = gas.cp * (T_3 - T_2);
    const Q_L = gas.cv * (T_4 - T_1);
    const W_net = Q_H - Q_L;
    const eta = W_net / Q_H;
    const etaIdeal = 1 - (1 / Math.pow(r, gas.k - 1)) * ((Math.pow(r_c, gas.k) - 1) / (gas.k * (r_c - 1)));
    ...
    return {
      ...
      derived: {
        cutoff_ratio: r_c,
        eta_thermal_ideal_formula: etaIdeal,
      },
      isReal,
    };
  } catch (error) {
    return calculatorError('Diesel', error);
  }
}
